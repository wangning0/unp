...
assert(ret!=-1);
setnonblocking(sig_pipefd[1]); // 客户端
addfd(epollfd,sig_pipefd[0]); // 服务端
addsig(SIGCHLD,sig_handler); // 在一个进程终止或者停止时，将SIGCHLD信号发送给其父进程
addsig(SIGTERM,sig_handler); // 请求中止进程,
addsig(SIGINT,sig_handler); // 程序终止(interrupt)信号, 在用户键入INTR字符(通常是Ctrl-C)时发出
addsig(SIGPIPE,SIG_IGN); // 管道破裂。这个信号通常在进程间通信产生，比如采用FIFO(管道)通信的两个进程，读管道没打开或者意外终止就往管道写，写进程会收到SIGPIPE信号  SIG_IGN忽略信号的处理程序
bool stop_server=false;
bool terminate=false;
/*创建共享内存，作为所有客户socket连接的读缓存*/
shmfd=shm_open(shm_name,O_CREAT|O_RDWR,0666);
assert(shmfd!=-1);
ret=ftruncate(shmfd,USER_LIMIT*BUFFER_SIZE); // ftruncate会将参数fd指定的文件大小改为参数length指定的大小
assert(ret!=-1);
